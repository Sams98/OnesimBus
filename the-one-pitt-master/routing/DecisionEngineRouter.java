...
public class DecisionEngineRouter extends ActiveRouter {
    ...
    public void changedConnection(Connection con) {
        ...
        if (con.isUp()) {
            ...
            for (Message m : msgs) {
                if (decider.shouldSendMessageToHost(m, myHost, otherNode)) //ini yang diubah
                {
    ...
    public Message messageTransferred(String id, DTNHost from) {
        ...
        if (incoming == null) {
            throw new SimError("No message with ID " + id + "from node = " + from + " in the incoming "
                    + "buffer of " + getHost());
        }
    ...
    protected void findConnectionsForNewMessage(Message m, DTNHost from) {
        ...
        for (Connection c : getConnections()) {
            ...
            DTNHost myHost = getHost(); //ini buat sendiri
            if (other != from && decider.shouldSendMessageToHost(m, myHost, other)) //ini yg diubah
            {
                //if(m.getId().equals("M7"))
